...
public abstract class AbstractQsbLayout extends FrameLayout implements OnSharedPreferenceChangeListener,
        OnLongClickListener, Insettable, SearchProviderController.OnProviderChangeListener {
...
//        this.Dn.setDelegateView(this.mMicIconView);
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        if (motionEvent.getActionMasked() == 0) {
            ...
            int i2 = 1;
            if (this.mIsRtl) {
                if (Float.compare(motionEvent.getX(), (float) (dI() ? getWidth() - this.Dk : 0)) >= 0) {
                    i = 1;
                }
            } else {
                if (Float.compare(motionEvent.getX(), (float) (dI() ? this.Dk : 0)) <= 0) {
                    i = 1;
                }
            }
            if (i == 0) {
