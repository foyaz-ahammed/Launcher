...
public class PredictionRowView extends PredictionsDividerLayout implements LogContainerProvider,
    ...
    public PredictionRowView(@NonNull Context context, @Nullable AttributeSet attributeSet) {
        ...
        mHeadingAppsLabel = getResources().getString(R.string.suggestion_app_label).toUpperCase();
    ...
    public void onColorChange(@NotNull ResolveInfo resolveInfo) {
        mIconTextColor = getContext().getColor(R.color.all_apps_text_color);
    ...
    public int getExpectedHeight() {
        ...
        return dp.allAppsCellHeightPx + getPaddingBottom() + getPaddingTop() + mHeadingLabelHeight;
    }
    ...
    private void applyPredictionApps() {
        ...
        if (getChildCount() != mNumPredictedAppsPerRow) {
            ...
            //Get label height, and set top margin same as it
            ...
            while (getChildCount() < mNumPredictedAppsPerRow) {
                ...
                bubbleTextView.setLongPressTimeout(Utilities.ALL_APPS_ICON_LONG_PRESS_DURATION);
                ...
                layoutParams.topMargin = (getExpectedHeight() - dp.allAppsCellHeightPx)/2;
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            bubbleTextView2.setTextColor(getContext().getColor(android.R.color.white));
        }
        if (size == 0) {
            if (mLoadingProgress == null) {
    ...
    protected void onDraw(Canvas canvas) {
        //Now draw the app label
        int color = getResources().getColor(R.color.apps_heading_label_text_color);
        mPaint.setColor(color);
        mPaint.setTextSize(mHeadingLabelTextSize);
        canvas.drawText(mHeadingAppsLabel, mHeadingLeftPadding, mHeadingTopPadding + mHeadingLabelTextSize, mPaint);

        //Now draw the background
        color = getResources().getColor(R.color.all_apps_background_color);
        mPaint.setColor(color);
        mPaint.setAlpha(40);
        canvas.drawRect(0, mHeadingLabelHeight, getWidth(), getHeight(), mPaint);
        mPaint.setAlpha(255);

        //Draw the line or all apps label, but do not draw it now
        if (mDividerType == DividerType.LINE) {
            /*
