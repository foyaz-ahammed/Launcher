...
public class LauncherStateManager {
    ...
    public void goToState(LauncherState state, boolean animated) {
        //Expand the page indicator at the right time

        if(animated && (state == LauncherState.OPTIONS)){
            mLauncher.getWorkspace().getPageIndicator().playAnimation(true, 300);
        }
        else if(state == NORMAL){
            if(animated)
                mLauncher.getWorkspace().getPageIndicator().playAnimation(false, 300);
            else {
                mLauncher.getWorkspace().removeExtraEmptyScreen(false, false);
                mLauncher.getWorkspace().getPageIndicator().setExpanded(false);
            }
        }

        //If the state is normal or all apps, the page spacing should be default one
        mLauncher.getWorkspace().setCustomPageSpacing(state == NORMAL || state == ALL_APPS);

        if(!animated) {

            //When animation is running, two animating children positions are same, so do not call request layout
            if(!mLauncher.getWorkspace().isTransitionAnimationRunning())
                mLauncher.getWorkspace().requestLayout();

            mLauncher.updateDrawCheck(state == OPTIONS);

            //Hotseat scale to original
            if(state == NORMAL) {
                Hotseat hotseat = mLauncher.getHotseat();
                hotseat.setScaleX(1);
                hotseat.setScaleY(1);

                //Move the label to the original position
                TextView workspaceDragLabel = mLauncher.getWorkspaceDragLabel();
                workspaceDragLabel.setAlpha(0);
                workspaceDragLabel.setTranslationY(-50);
    ...
    private void goToState(LauncherState state, boolean animated, long delay,
        ...
        animation.addListener(new AnimatorListener(){
            ...
            public void onAnimationEnd(Animator animation) {
                if(fromState == NORMAL && mToState == OPTIONS){
                    //Draw the check icon
                    mLauncher.updateDrawCheck(true);
                }
                else if(fromState == OPTIONS && mToState == NORMAL){
                    //Remove all drawn check icons
                    mLauncher.updateDrawCheck(false);
                }
            }
    ...
    private void onStateTransitionStart(LauncherState state) {
        ...
        //Move the app dock to original position
        if(mState == OPTIONS && state == NORMAL){
            mLauncher.moveHotseatToOriginalPosition();
        }
