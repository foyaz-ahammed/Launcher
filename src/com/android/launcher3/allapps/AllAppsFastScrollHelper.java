...
public class AllAppsFastScrollHelper implements AllAppsGridAdapter.BindViewCallback {
    ...
    public boolean smoothScrollToSection(int scrollY, int availableScrollHeight,
            int position, String sectionName, boolean scrollToTop) {
        int tempPosition = position;

        //If scroll to top temp position is not necessary, so set it unused value
        if(scrollToTop)
            tempPosition = -2;
        if (mTargetFastScrollPosition != tempPosition) {
            mTargetFastScrollPosition = tempPosition;
            smoothSnapToPosition(scrollY, availableScrollHeight, position, sectionName, scrollToTop);
            return true;
        }
        return false;
    }
    ...
    private void smoothSnapToPosition(int scrollY, int availableScrollHeight,
        ...
        if(!scrollToTop) {
            if (mHasFastScrollTouchSettled) {
                ...
                mCurrentFastScrollSection = sectionName;
                ...
                mTargetFastScrollSection = sectionName;
        ...
        int paddingTop = mRv.getPaddingTop();
        int newScrollY = Math.min(availableScrollHeight, mRv.getCurrentScrollY(Math.max(0,position), 0) + paddingTop);
        if(scrollToTop)
            newScrollY = 0;
    ...
    private void updateTrackedViewsFastScrollFocusState() {
        for (RecyclerView.ViewHolder viewHolder : mTrackedFastScrollViews) {
            ...
            if (mCurrentFastScrollSection != null
                ...
                isActive = item != null && item.appInfo != null &&
                        mCurrentFastScrollSection.equals(mApps.getIndexer().computeSectionName(item.appInfo.title)) &&
