...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    public void completeDrop(DragObject d) {
        ...
        if (canRemove(item)) {
            ...
            //Remove the items in group
            GroupIconManager groupIconManager = mLauncher.getDragController().getGroupIconManager();
            for (int i = 0; i < groupIconManager.getGroupIconCount(); i ++){
                GroupIconCell iconCell = groupIconManager.getGroupIconCell(i);
                ItemInfo itemInfo = (ItemInfo) iconCell.getCellView().getTag();
                if(itemInfo != null && canRemove(itemInfo)){
