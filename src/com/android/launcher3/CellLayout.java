...
/**
 * Workspace에서 차지하는 한개 페지 Layout
 * App아이콘, 등록부, Widget들을 가지고 있다.
 *
 * 자식 view로서는 {@link ShortcutAndWidgetContainer} 하나만 존재하며 이 view에 shortcut(icon, folder), widget들이 들어있다.
 * @see Workspace
 */
public class CellLayout extends ViewGroup {
    public static final int WORKSPACE_ACCESSIBILITY_DRAG = 2;
    public static final int FOLDER_ACCESSIBILITY_DRAG = 1;
    public static final float CAMERA_DISTANCE_SCALE = 20;
    ...
    //전환 Animation을 진행하기 전에 림시로 보관해두는 왼쪽, 오른쪽위치
    private int mTempLeft;
    private int mTempRight;

    //전환 Animation을 진행할때 설정해주는 왼쪽, 오른쪽 위치
    private int mAnimateLeft;
    private int mAnimateRight;
    ...
    public void removeAllAndSave(){
        ...
        for (int i = 0; i < mShortcutsAndWidgets.getChildCount(); i ++){
            ...
            if(view.getTag() instanceof ItemInfo){
                mLauncher.getModelWriter().deleteItemFromDatabase((ItemInfo)view.getTag());
            }
        }
        mShortcutsAndWidgets.removeAllViews();
    }
    ...
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration,
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            child.setTag(R.string.group_drag_animate_to_pos, va);
    ...
    void visualizeDropLocation(View v, DragPreviewProvider outlineProvider, int cellX, int cellY,
        ...
        if (cellX != oldDragCellX || cellY != oldDragCellY) {
            ...
            if (resize) {
                ...
                //Set a new rectangle position
                if(isHotseat()){
                    int childCount = getHotSeatChildCount();
                    if(!(v != null && v.getTag() != null && v.getTag() instanceof ItemInfo &&
                            ((ItemInfo)v.getTag()).container == Favorites.CONTAINER_HOTSEAT)){
                        childCount++;
                    }
                    int startPos = ShortcutAndWidgetContainer.getHotSeatChildStartPos(cellX, mCellWidth, childCount);
                    left = startPos + getPaddingLeft();
                }
    ...
    private int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX,
        ...
        int hotSetChildCount = getHotSeatChildCount();
        if(mLauncher.getWorkspace().isDragDesktopToHotSeat()){
            if(hotSetChildCount < MAX_COUNT_HOTSEAT){
                hotSetChildCount ++;
            }
        }
        ...
        for (int y = 0; y < countY - (minSpanY - 1); y++) {
            ...
            for (int x = 0; x < countX - (minSpanX - 1); x++) {
                ...
                if(isHotseat()) {
                    cellXY[0] = getPaddingLeft() + ShortcutAndWidgetContainer.getHotSeatChildStartPos(x,
                            mCellWidth, hotSetChildCount) + (spanX * mCellWidth) / 2;
                }
    ...
    */
    ...
    public int getHotSeatChildCount(){
        if(isHotseat())
            return mShortcutsAndWidgets.getChildCount();
        return 0;
    }

    public void onHotSeatToHotSeatDragOver(int sourceCellX, int targetIndex){
        int sourceIndex = mShortcutsAndWidgets.getColumnIndex(sourceCellX, 0);
        int childCount = getHotSeatChildCount();
        for (int i = 0; i < childCount; i ++){
            View view = mShortcutsAndWidgets.getChildAt(i);
            int columnIndex = mShortcutsAndWidgets.getColumnIndex(((LayoutParams)view.getLayoutParams()).cellX, 0);
            ...
            else if(columnIndex < Math.min(sourceIndex, targetIndex) || columnIndex > Math.max(sourceIndex, targetIndex)){
                newIndex = columnIndex;
            }
            ...
            int pos = ShortcutAndWidgetContainer.getHotSeatChildStartPos(columnIndex, mCellWidth, childCount);
            int newPos = ShortcutAndWidgetContainer.getHotSeatChildStartPos(newIndex, mCellWidth, childCount);

            if(view.getTag(R.string.cell_animator_tag) != null){
                ObjectAnimator oldAnimator = (ObjectAnimator)view.getTag(R.string.cell_animator_tag);
                oldAnimator.cancel();
            }

            ObjectAnimator animator = ObjectAnimator.ofFloat(view, "translationX", view.getTranslationX(), newPos - pos);
            animator.setDuration(300);
            animator.start();

            view.setTag(R.string.cell_animator_tag, animator);
        }
    }

    public void onDesktopToHotSeatDragOver(int index){
        int childCount = getHotSeatChildCount();
        for (int i = 0; i < childCount; i ++){
            View view = mShortcutsAndWidgets.getChildAt(i);
            int columnIndex = mShortcutsAndWidgets.getColumnIndex(((LayoutParams)view.getLayoutParams()).cellX, 0);
            ...
            int pos = ShortcutAndWidgetContainer.getHotSeatChildStartPos(columnIndex, mCellWidth, childCount);
            int newPos = ShortcutAndWidgetContainer.getHotSeatChildStartPos(newIndex, mCellWidth, childCount + 1);
    ...
    public void onDesktopToHotSeatDragLeave(){
        int childCount = getHotSeatChildCount();
        for (int i = 0; i < childCount; i ++){
            View view = mShortcutsAndWidgets.getChildAt(i);
    ...
    public void onHotSeatToHotSeatDragLeave(int sourceCellX){
        int sourceIndex = mShortcutsAndWidgets.getColumnIndex(sourceCellX, 0);
        int childCount = getHotSeatChildCount();
        for (int i = 0; i < childCount; i ++){
            View view = mShortcutsAndWidgets.getChildAt(i);
    ...
    /*-- Transition Animation 에 리용되는 속성함수들 --*/
    //Cube
    @Keep
    public void setCube(float fraction) {
        float translationX = getOnePageScrollWidth() * fraction;
    ...
    public void setZoomSlideHorizontal(float fraction) {
        setTranslationX(getOnePageScrollWidth() * fraction);
    ...
    /**
     * View의 왼쪽, 오른쪽 위치보관
     * (전환 animation을 진행하기전에 View를 다른 위치로 이동시키기때문에 본래 위치를 림시로 보관해둔다.)
     */
    ...
    /**
     * Animation을 진행하기전에 오른쪽, 왼쪽 위치를 변경한다.
     * 이 함수에서는 그 위치를 먼저 변수에 보관해둔다.
     *
     * @param left 왼쪽위치
     * @param right 오른쪽위치
     */
    public void setAnimatePosition(int left, int right){
        mAnimateLeft = left;
        mAnimateRight = right;
    }

    /**
     * 현재 View의 왼쪽, 오른쪽 위치를 설정한다.
     * {@link #mAnimateLeft}, {@link #mAnimateRight}
     */
    public void setLayoutPosition(){
        if(getLeft() != mAnimateLeft || getRight() != mAnimateRight){
            setLeft(mAnimateLeft);
            setRight(mAnimateRight);
        }
    }

    /**
     * Animation을 진행하면서 변경되였던 View속성을 초기화한다.
     */
    public void initializeProperties(){
        //Translation, Rotation, Scale, Alpha값들을 초기화한다.
        setTranslationX(0);
        setRotationX(0);
        setRotationY(0);
        setRotation(0);
    ...
    /**
     * 한개 페지({@link CellLayout})가 끌기를 통해 이동하는 거리
     */
    private int getOnePageScrollWidth(){
        return mLauncher.getWorkspace().getOnePageScrollWidth();
    }

    /**
     * 자식 View({@link BubbleTextView})들의 animation을 모두 중지시킨다.
     */
    public void stopAnimations(){
        int childCount = getHotSeatChildCount();
        for (int i = 0; i < childCount; i ++){
            View view = mShortcutsAndWidgets.getChildAt(i);
            ObjectAnimator animator = (ObjectAnimator)view.getTag(R.string.cell_animator_tag);
            if(animator != null) {
                animator.cancel();
            }
            view.clearAnimation();
    ...
    /**
     * 자식 View({@link BubbleTextView})가 없는 빈 페지인가? (혹은 Hotseat)
     */
    ...
    /**
     * 현재의 CellLayout이 Hotseat인 경우 빈 자리들을 없앤다. (실레: 1 x 2 3 x -> 1 2 3)
     */
    public void removeEmptySpaceOnHotSeat(){
        mShortcutsAndWidgets.removeEmptySpaceOnHotSeat();
    }
}
