...
public class FolderAnimationManager {
    ...
    private GradientDrawable mFolderMainViewBackground;
    ...
    public FolderAnimationManager(Folder folder, boolean isOpening) {
        ...
        mFolderMainViewBackground = (GradientDrawable) mFolder.getMainFooterView().getBackground();
     ...
     */
    public AnimatorSet getAnimator(Rect []rectPos) {
        ...
        float scaleRelativeToDragLayer = mLauncher.getDragLayer()
                .getDescendantRectRTF(mFolderIcon, folderIconPos, mLauncher.isInState(LauncherState.NORMAL));

        if(mIsOpening){
            rectPos[0] = folderIconPos;
        }
        else{
            folderIconPos = rectPos[0];
        }
        ...
        float previewScale = rule.scaleForItem(Math.max(1, itemsInPreview.size()));
        float previewSize = rule.getIconSize() * previewScale;
        float initialScale;

        int iconSize = mLauncher.getDeviceProfile().folderIconOffsetYPx * 2 + mLauncher.getDeviceProfile().folderIconSizePx;
        initialScale = previewSize / iconSize * scaleRelativeToDragLayer;

        float finalScale = 1f;

        //Set scale 0.8f on options or spring loaded state
        ...
        float workspaceScale = mLauncher.getWorkspace().getScaleX();

        //If the current state is normal, workspace is scaled down, but calculations should be done by purposing
        //workspace is not scaled
        ...
        int previewItemOffsetX = (int) (previewSize * workspaceScale / 2);
        if (Utilities.isRtl(mContext.getResources())) {
            previewItemOffsetX = (int) ((lp.width * initialScale - initialSize - previewItemOffsetX)*workspaceScale);
        }

        final int paddingOffsetX = (int) ((mFolder.getPaddingLeft() + mContent.getPaddingLeft())
                * initialScale * workspaceScale);
        final int paddingOffsetY = (int) ((mFolder.getPaddingTop() + mContent.getPaddingTop())
                * initialScale * workspaceScale);

        int initialX = folderIconPos.left + (int)(mPreviewBackground.getOffsetX() * workspaceScale) - paddingOffsetX
                - previewItemOffsetX;
        int initialY = folderIconPos.top + (int)(mPreviewBackground.getOffsetY() * workspaceScale) - paddingOffsetY;
        ...
        //Change the final color(white -> dark gray)
//        int finalColor = Themes.getAttrColor(mContext, android.R.attr.colorPrimary);

        final int finalColor = mLauncher.getColor(
                mLauncher.isInState(LauncherState.NORMAL)?
                        R.color.folder_view_big_background_color : R.color.folder_view_background_color);

        final int initialColor = mPreviewBackground.getBgColor();
//                ColorUtils.setAlphaComponent(finalColor, mPreviewBackground.getBackgroundAlpha());

        mFolderMainViewBackground.mutate();
        mFolderMainViewBackground.setColor(mIsOpening ? initialColor : finalColor);
        ...
        PropertyResetListener<BubbleTextView, Float> colorResetListener =
        ...
        int initialTranslationY = 0;
        EditText folderName = mFolder.getFolderNameView();
        if(folderName != null) {
            if(folderName.getHeight() == 0) {
                folderName.measure(0, 0);
                initialTranslationY = -folderName.getMeasuredHeight();
            }
            else{
                initialTranslationY = -folderName.getHeight();
            }
        }

        play(a, getAnimator(mFolder, View.TRANSLATION_X, xDistance, 0));
        play(a, getAnimator(mFolder, View.TRANSLATION_Y, yDistance, 0));
        play(a, getAnimator(mFolder.getMainFooterView(), View.TRANSLATION_Y, initialTranslationY, 0));

        //In normal state, check icon is not drawn
        if(!mLauncher.isInState(LauncherState.NORMAL)) {
            play(a, getAnimator(mContent, "checkIconAlpha", 0, 1));
        }

        play(a, getAnimator(mFolder, SCALE_PROPERTY, initialScale, finalScale));
        play(a, getAnimator(mFolderMainViewBackground, "color", initialColor, finalColor));
        play(a, mFolderIcon.mFolderName.createTextAlphaAnimator(!mIsOpening));

        play(a, getAnimator(mFolder.getFolderNameView(), View.ALPHA, 0, 1));
...
//                mFolder.setScaleY(1f);
        ...
        int radiusOffset = Math.round(totalOffsetX*(1 - workspaceScale)/2);
        ...
        addPreviewItemAnimators(a, initialScale / scaleRelativeToDragLayer,
                ...
                previewItemOffsetX + radiusDiff + radiusOffset, radiusDiff - radiusOffset);
    ...
    private Animator getAnimator(View view, String property, float v1, float v2) {
        return mIsOpening
                ? ObjectAnimator.ofFloat(view, property, v1, v2)
                : ObjectAnimator.ofFloat(view, property, v2, v1);
    }
