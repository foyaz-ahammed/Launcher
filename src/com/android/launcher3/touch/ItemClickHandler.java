...
public class ItemClickHandler {
    ...
    private static void onClick(View v) {
        ...
        if (launcher.getWorkspace().isTransitionAnimationRunning()) {
            return;
        }
        ...
        if (tag instanceof ShortcutInfo) {
            ...
            boolean canOpenApp = false;

            //First check it is from folder
            if(v.getParent().getParent().getParent().getParent().getParent() instanceof Folder){
                Folder folder = (Folder) v.getParent().getParent().getParent().getParent().getParent();
                //Check it is the case to open the app or not
                canOpenApp = !(folder.isInDesktop() && launcher.drawCheck());

                if(canOpenApp){
                    launcher.folderViewAnimation(false);
                }
            }
            else {
                //Check it is the case to open the app or not
                canOpenApp = !(info.isContainerDesktop() && launcher.drawCheck());
            }

            //Open app, or toggle check
            if (canOpenApp)
    ...
    private static void onClickFolderIcon(View v) {
        ...
        if (!folder.isOpen() && !folder.isDestroyed()) {
            ...
            //Insert add button to the folder
            folder.insertAddIcon();
        }
    }
