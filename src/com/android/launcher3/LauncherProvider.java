...
public class LauncherProvider extends ContentProvider {
    ...
    public static class DatabaseHelper extends NoLocaleSQLiteHelper implements LayoutParserCallback {
        ...
        DatabaseHelper(Context context, Handler widgetHostResetHandler) {
            ...
            if(!tableExists(QuickAccess.TABLE_NAME)){
                addQuickAccessTable(getWritableDatabase(), true);
            }

            if(!tableExists(AssistantSectionOrder.TABLE_NAME)){
                addAssistantSectionOrderTable(getWritableDatabase(), true);
            }

            if (!tableExists(SearchHistory.TABLE_NAME)) {
                addSearchHistoryTable(getWritableDatabase(), true);
            }
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            addQuickAccessTable(db, true);
            addAssistantSectionOrderTable(db, true);
            addSearchHistoryTable(db, true);
        ...
        private void addQuickAccessTable(SQLiteDatabase db, boolean optional) {
            QuickAccess.addTableToDb(db, optional);
        }

        private void addAssistantSectionOrderTable(SQLiteDatabase db, boolean optional) {
            AssistantSectionOrder.addTableToDb(db, optional);
        }

        private void addSearchHistoryTable(SQLiteDatabase db, boolean optional) {
            SearchHistory.addTableToDb(db, optional);
        }
