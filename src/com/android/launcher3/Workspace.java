...
import static com.android.launcher3.ShortcutAndWidgetContainer.MAX_COUNT_HOTSEAT;
...
import com.android.launcher3.transition.TransitionLayout;
import com.android.launcher3.transition.TransitionManager;
import com.android.launcher3.transition.TransitionUtility;
...
/**
 * Wallpaper(배경화상)와 여러개의 페지를 가지고 있다.
 * 매 페지에는 App 아이콘, 등록부, Widget들이 있다.
 *
 * @see CellLayout (한개 페지)
 */
public class Workspace extends PagedView<WorkspacePageIndicator>
    ...
    //Variables for dragging to hotseat
    private boolean mIsDragDesktopToHotSeat = false;
    private boolean mIsDragHotSeatToHotSeat = false;
    private int mDragHotSeatIndex = 0;

    //A flag value that shows it passed action move
    private boolean mHasHandledMove = false;

    //Default page spacing
    private int mDefaultPageSpacing = 0;

    //Variables when scrolling for transition effects
    private static final float RETURN_TO_ORIGINAL_PAGE_THRESHOLD_NOT_FIRST = 0.47f;
    // The page is moved more than halfway, automatically move to the next page on touch up.
    private static final float SIGNIFICANT_MOVE_THRESHOLD_NOT_FIRST = 0.54f;

    //Top parent Desktop window
    private Desktop mDesktop;

    private final TransitionManager mTransitionManager;

    protected float mLastOffsetX = 0;

    /**
     * 3개의 animation(첫 페지, 둘째 페지, 배경화상)들에 해당한 animator
     */
    private AnimatorSet mFirstAnimatorSet = null;   //첫페지
    private AnimatorSet mSecondAnimatorSet = null;  //둘째페지
    private ValueAnimator mWallpaperMoveAnimator = null;    //배경화상

    //Animation에 참가하는 View들
    CellLayout mAnimateFirstView = null;    //첫페지
    CellLayout mAnimateSecondView = null;   //둘째페지

    private int mCurrentX;  //현재 위치의 scroll값
    private float mUpFraction = 0;  //끌기하다가 놓았을때 보관해두는 Animation속성값

    private int mLastTouchAction = -1;  //OnTouchEvent에서 매번 보관하는 Action

    private int mRunningAnimatorCount = 0;  //현재 진행중인 Animator의 개수(우에서 정의 한 3가지 animator)

    private boolean mTouchInterceptAnimation = false;
    private boolean mWasOnScrollToAssistant = false;
    ...
    public void setInsets(Rect insets) {
        ...
        if (grid.shouldFadeAdjacentWorkspaceScreens()) {
            ...
            mDefaultPageSpacing = grid.defaultPageSpacingPx;
        } else {
            // In portrait, we want the pages spaced such that there is no
            // overhang of the previous / next page into the current page viewport.
            // We assume symmetrical padding in portrait mode.
            mDefaultPageSpacing = Math.max(grid.defaultPageSpacingPx, padding.left + 1);
        }
        setCustomPageSpacing(true);
        requestLayout();
    ...
    public void setCustomPageSpacing(boolean normalMode){
        if(normalMode)
            setPageSpacing(mDefaultPageSpacing);
        else {
            DeviceProfile profile = mLauncher.getDeviceProfile();
            int cellWidth = profile.getCellSize().x;
            int fullWidth = getResources().getDisplayMetrics().widthPixels;
            int iconWidth = profile.iconSizePx;
            int numColumns = profile.inv.numColumns;
            int cellPadding = profile.cellLayoutPaddingLeftRightPx;
            float scale = profile.workspaceOptionsShrinkFactor;

            //Full width minus scaled child width
            int leftSpace = (int)((fullWidth - (cellWidth*numColumns + cellPadding*2) * scale)*0.5f);
            int cellRightSpace = (int)(((cellWidth - iconWidth)*0.5f + cellPadding)*scale);
            int calculatedPageSpacing = leftSpace - cellRightSpace;

            //float value can have offset, so plus 2
            setPageSpacing(calculatedPageSpacing + 2);
        }
    }
    ...
    public void onDragStart(DropTarget.DragObject dragObject, DragOptions options) {
        ...
        //Check last state is all apps state, and then automatically go to options state
     ...
     * Touch사건동작을 처리한다.
     *
     * 페지간 전환 ainmation을 진행한다.
     * 화면을 끌기하는동안에는 화면을 움직여주고 끌기를 놓은다음에는 animation을 통해 페지전환을 진행한다.
     * Multi touch, 여러번 touch(Animation이 끝나기전에 touch사건이 일어날때에는 animation을 중지한다.)
     * 홈설정에서 보면 8가지 형식의 전환형식이 있다.
     *
     * @param ev MotionEvent
     *
     * @see TransitionManager
     * @see TransitionUtility
     */
    @Override
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                //이미 animation이 진행중일때?
                if((mFirstAnimatorSet != null) || (mSecondAnimatorSet != null)) {
                    mTouchInterceptAnimation = true;

                    //Animator중지
                    if(mWallpaperMoveAnimator != null){
                        mWallpaperMoveAnimator.pause();

                        //Animation 진행하는동안 페지들이 어느만큼 움직였는가를 계산한다.
                        float downFraction = mWallpaperMoveAnimator.getAnimatedFraction();
                        float movedDistance = getOnePageScrollWidth() * (downFraction - mUpFraction) * (mLastOffsetX <= 0? -1 : 1);

                        //움직인 거리를 더해준다,
                        mCurrentX = mCurrentX + (int)movedDistance;

                        if(mWasInOverscroll) {  //OverScroll 진행중이였는가? 이때는 수축된거리를 계산하여 더해준다.
                            int scrollOffset = Math.abs(mCurrentX - mUnboundedScrollX);
                            int invertDampAmount = OverScroll.invertedDampScroll(scrollOffset, getMeasuredWidth());
                            invertDampAmount = invertDampAmount * (mLastOffsetX <= 0? -1 : 1);
                            mCurrentX = mUnboundedScrollX + invertDampAmount;
                        }
                    }

                    //Animator중지
                    if(mFirstAnimatorSet != null) {
                        mFirstAnimatorSet.pause();
                    }
                    if(mSecondAnimatorSet != null) {
                        mSecondAnimatorSet.pause();
                    }
                }

                //그렇지 않을때
                else {
                    mCurrentX = mUnboundedScrollX;
                }
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    //1 pixel 이상 움직였을때만 scroll을 진행한다.
                    if (Math.abs(deltaX) >= 1.0f) {
                        if(mFirstAnimatorSet != null && mFirstAnimatorSet.isPaused()){
                            mFirstAnimatorSet.removeAllListeners();
                            mFirstAnimatorSet = null;

                            if(mAnimateFirstView != null){
                                mAnimateFirstView.restoreState();
                            }
                        }

                        if(mSecondAnimatorSet != null && mSecondAnimatorSet.isPaused()){
                            mSecondAnimatorSet.removeAllListeners();
                            mSecondAnimatorSet = null;

                            if(mAnimateSecondView != null){
                                mAnimateSecondView.restoreState();
                            }
                        }

                        if(mWallpaperMoveAnimator != null && mWallpaperMoveAnimator.isPaused()){
                            mWallpaperMoveAnimator.removeAllListeners();
                            mWallpaperMoveAnimator = null;
                        }

                        //화면에서 좌표, scroll위치들을 보관한다.
                        mLastMotionX = x;
                        mLastMotionXRemainder = deltaX - (int) deltaX;
                        mCurrentX = (int) (mCurrentX - deltaX);

                        final int onePageWidth = getOnePageScrollWidth();
                        float offsetX = mCurrentX - mUnboundedScrollX; //편차값 (절대값은 한페지너비를 넘지 않는다.)

                        //Scroll을 진행하여 한페지 이상 지나갔을때
                        if(Math.abs(offsetX) >= onePageWidth && !mWasInOverscroll){
                            //현재 진행중이던 animation들을 중지한다.
                            if(mWallpaperMoveAnimator != null){
                                mWallpaperMoveAnimator.pause();
                                mWallpaperMoveAnimator = null;
                            }
                            ...
                            if(offsetX > 0) {   //전페지를 현재 페지로 설정
                                setCurrentPage(validateNewPage(mCurrentPage - 1));
                                offsetX -= onePageWidth;
                            } else {       //다음페지를 현재 페지로 설정
                                setCurrentPage(validateNewPage(mCurrentPage + 1));
                                offsetX += onePageWidth;
                            }

                            //그다음 mCurrentX를 갱신한다.
                            mCurrentX = (int) (mUnboundedScrollX + offsetX);
                        }

                        //이전의 Scroll 방향
                        final boolean lastDirectionForward = mLastOffsetX <= 0;
                        //현재의 Scroll 방향
                        final boolean currentDirectionForward = offsetX <= 0;

                        //방향이 서로 다르다면 이전의 animation을 중지시킨다.
                        if(lastDirectionForward != currentDirectionForward){
                            if(mFirstAnimatorSet != null){
                                mFirstAnimatorSet.end();
                                mFirstAnimatorSet = null;
                                if(mAnimateFirstView != null){
                                    mAnimateFirstView.restoreState();
                                }
                        ...
                        //Scroll위치
                        float measuredScrollX = getUnboundedScrollX() - offsetX;
                        //화면이 첫페지에서 왼쪽으로 움직였는가? (Assistant 페지로 scroll)
                        boolean isXBeforeFirstPage = mIsRtl ? (measuredScrollX > mMaxScrollX) : (measuredScrollX < 0);
                        //화면이 마지막페지에서 오른쪽으로 움직였는가? (OverScroll)
                        boolean isXAfterLastPage = mIsRtl ? (measuredScrollX < 0) : (measuredScrollX > mMaxScrollX);

                        //Assistant 페지로 scroll?
                        mWasOnScrollToAssistant = isXBeforeFirstPage && mLauncher.showAssistantPage();

                        /*-- OverScroll인 경우 offset값을 다시 계산한다. --*/
                        if(isXBeforeFirstPage) {
                            mWasInOverscroll = true;
                            if(!mWasOnScrollToAssistant) {
                                offsetX = OverScroll.dampedScroll(offsetX, getMeasuredWidth());
                            }
                        ...
                        //offsetx로부터 offsetValue를 계산한다. (이 값은 0~1로서 animation의 속성값으로 리용된다.)
                        final float offsetValue = Math.abs(offsetX)/onePageWidth;

                        //Animator 창조
                        if(!mWasOnScrollToAssistant && (mFirstAnimatorSet == null || mSecondAnimatorSet == null)) {
                            //Animation resource 들을 불러온다.
                            final int transition_left_in = mTransitionManager.getAnimationLeftIn();
                            final int transition_left_out = mTransitionManager.getAnimationLeftOut();
                            final int transition_right_in = mTransitionManager.getAnimationRightIn();
                            final int transition_right_out = mTransitionManager.getAnimationRightOut();

                            if(!isXBeforeFirstPage && mFirstAnimatorSet == null) {
                                mAnimateFirstView =
                                        (CellLayout) (currentDirectionForward ? getPageAt(mCurrentPage)
                                                                                        : getPageAt(mCurrentPage - 1));
                                mFirstAnimatorSet = (AnimatorSet) AnimatorInflater
                                        .loadAnimator(getContext(),
                                                currentDirectionForward ? transition_left_out : transition_left_in );
                                mFirstAnimatorSet.setTarget(mAnimateFirstView);

                                //Animation을 진행하기 전에 왼쪽, 오른쪽 위치를 보관한다.
                                if(mAnimateFirstView != null) {
                                    mAnimateFirstView.savePosition();
                                }
                            }

                            if(!isXAfterLastPage && mSecondAnimatorSet == null) {
                                mAnimateSecondView =
                                        (CellLayout) (currentDirectionForward ? getPageAt(mCurrentPage + 1)
                                                                                        : getPageAt(mCurrentPage));
                                mSecondAnimatorSet = (AnimatorSet) AnimatorInflater
                                        .loadAnimator(getContext(),
                                                currentDirectionForward ? transition_right_in : transition_right_out);
                                mSecondAnimatorSet.setTarget(mAnimateSecondView);

                                //Animation을 진행하기 전에 왼쪽, 오른쪽 위치를 보관한다.
                                if(mAnimateSecondView != null) {
                                    mAnimateSecondView.savePosition();
                                }
                            }

                            if(mWallpaperMoveAnimator == null) {
                                mWallpaperMoveAnimator = ValueAnimator.ofFloat(0, 1);
                                mWallpaperMoveAnimator
                                            ...
                                            public void onAnimationUpdate(ValueAnimator animation) {
                                                float value =
                                                        (Float) animation.getAnimatedValue() * (
                                                                mLastOffsetX <= 0 ? -1 : 1);
                                                mLastOffsetX = value * onePageWidth;
                                                mWallpaperOffset.syncWithScroll();

                                                //Set left, and right positions of animating views
                                                if(mAnimateFirstView != null && indexOfChild(mAnimateFirstView) != mCurrentPage){
                                                    mAnimateFirstView.setLayoutPosition();
                                                }
                                                if(mAnimateSecondView != null && indexOfChild(mAnimateSecondView) != mCurrentPage){
                                                    mAnimateSecondView.setLayoutPosition();
                                                }
                                            }
                                        });
                                mWallpaperMoveAnimator.setInterpolator(new LinearInterpolator());
                                mWallpaperMoveAnimator.setDuration(getResources()
                                        .getInteger(R.integer.transition_common_duration));
                            }

                            //Set same start position of first and second animate view
                            if(!mWasInOverscroll && mAnimateFirstView != null && mAnimateSecondView!= null) {
                                if (currentDirectionForward) {
                                    mAnimateSecondView.setAnimatePosition(mAnimateFirstView.getLeft(), mAnimateFirstView.getRight());
                                    mAnimateSecondView.setLeft(mAnimateFirstView.getLeft());
                                    mAnimateSecondView.setRight(mAnimateFirstView.getRight());

                                } else {
                                    mAnimateFirstView.setAnimatePosition(mAnimateSecondView.getLeft(), mAnimateSecondView.getRight());
                                    mAnimateFirstView.setLeft(mAnimateSecondView.getLeft());
                                    mAnimateFirstView.setRight(mAnimateSecondView.getRight());
                                }
                            }
                        }
                        ...
                        if(mWasOnScrollToAssistant) { //Assistant페지로 scroll
                            if(mCurrentPage > 0) setCurrentPage(0); //첫페지를 현재 페지로 설정
                            mDesktop.moveWithOffset((int)offsetX);  //Desktop layout을 이동시킨다.
                        } else {    //일반 scroll
                            //Animator들의 위치를 설정한다. (이런 방식으로 화면을 움직인다.)
                            if (mFirstAnimatorSet != null) {    //첫페지
                                //AnimatorSet에서는 재생시간을 계산하여 설정한다.
                                final long totalDuration = mFirstAnimatorSet.getTotalDuration();
                                final float calculatedTime = totalDuration * offsetValue;
                                mFirstAnimatorSet.setCurrentPlayTime((long) calculatedTime);
                            }
                            if (mSecondAnimatorSet != null) {   //둘째페지
                                //AnimatorSet에서는 재생시간을 계산하여 설정한다.
                                final long totalDuration = mSecondAnimatorSet.getTotalDuration();
                                final float calculatedTime = totalDuration * offsetValue;
                                mSecondAnimatorSet.setCurrentPlayTime((long) calculatedTime);
                            }
                            if (mWallpaperMoveAnimator != null) {   //배경화상
                                //Animation속성값을 설정해준다.
                                mWallpaperMoveAnimator.setCurrentFraction(offsetValue);
                            }
                        }

                        mHasHandledMove = true;
                    } else {
                        awakenScrollBars();
                    }
                } else {
                ...
                break;
            case MotionEvent.ACTION_UP: //끌기끝
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    //ACTION_CANCEL이 먼저 호출되여 같은 동작이 이미 진행되였을수 있으므로
                    //검사하고 같은 동작이 두번 진행되지 않도록 한다.
                    if(mLastTouchAction == MotionEvent.ACTION_UP){
                        break;
                    }

                    //ACTION_MOVE를 거치지 않고 곧장 ACTION_UP에 들어왔을때는
                    //중지했던 animation들을 다시 실행시킨다.
                    if(!mHasHandledMove) {
                        if(mFirstAnimatorSet != null && mFirstAnimatorSet.isPaused())
                            mFirstAnimatorSet.resume();
                        if(mSecondAnimatorSet != null && mSecondAnimatorSet.isPaused())
                            mSecondAnimatorSet.resume();
                        if(mWallpaperMoveAnimator != null && mWallpaperMoveAnimator.isPaused())
                            mWallpaperMoveAnimator.resume();
                        break;
                    }
                    mHasHandledMove = false;

                    //Animation 속성값을 보관해둔다.
                    if(mWallpaperMoveAnimator != null) {
                        mUpFraction = mWallpaperMoveAnimator.getAnimatedFraction();
                    }
                    ...
                    final int deltaX = (int) mLastOffsetX;
                    final int pageWidth = getPageAt(mCurrentPage).getMeasuredWidth();
                    final float significant_move_threshold = mTouchInterceptAnimation? SIGNIFICANT_MOVE_THRESHOLD_NOT_FIRST : SIGNIFICANT_MOVE_THRESHOLD;
                    final float return_to_original_page_threshold = mTouchInterceptAnimation? RETURN_TO_ORIGINAL_PAGE_THRESHOLD_NOT_FIRST : RETURN_TO_ORIGINAL_PAGE_THRESHOLD;
                    final boolean isSignificantMove = Math.abs(deltaX) > pageWidth * significant_move_threshold;
                    final boolean isFling = mTotalMotionX > mTouchSlop && shouldFlingForVelocity(velocityX);

                    if (!mFreeScroll) {
                        boolean returnToOriginalPage = false; //본래 페지로 되돌아오는가?
                        ...
                        if(mWasInOverscroll) {
                            if(mWasOnScrollToAssistant){
                                ...
                                //현재 scroll위치를 다시 계산한다.
                                mCurrentX = (int) (mUnboundedScrollX + mLastOffsetX);
                            }
                        }
                        ...
                        if(mWasOnScrollToAssistant) { //Assist page로 scroll
                            //이미 animation이 진행중이면 중지시킨다.
                            if(mSecondAnimatorSet != null) {
                                mSecondAnimatorSet.removeAllListeners();
                                mSecondAnimatorSet = null;
                            }

                            //Desktop을 이동하는 animation을 시작한다.
                            mDesktop.playCurrentAnimation(isFling, isVelocityXLeft);
                        } else { //Workspace내에서의 scroll
                            //목표페지위치를 계산하고 animation을 진행한다.
                            int finalPage;
                            if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                                    (isFling && !isVelocityXLeft)) && mCurrentPage > 0) {
                                finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1;
                                if (returnToOriginalPage) {
                                    cancelCurrentTransitionAnimation();
                                } else {
                                    if (mLastOffsetX <= 0) {
                                        cancelCurrentTransitionAnimation();
                                    } else {
                                        finishCurrentTransitionAnimation(finalPage);
                                    }
                                }
            ...
            case MotionEvent.ACTION_CANCEL:
                //이때는 ACTION_UP 에서와 꼭같은 동작을 진행한다.
        ...
        mLastTouchAction = ev.getAction();
        return true;
    }

    public int getOnePageScrollWidth(){
        int childWidth = 0;
        if(getChildAt(0) != null)
            childWidth = getChildAt(0).getWidth();

        return childWidth + mPageSpacing + getChildGap();
    }
    ...
    public void onDragEnd() {
        ...
        if (!mDeferRemoveExtraEmptyScreen) {
            //Check current state is not options state and then remove empty screen
            if(!mLauncher.isInState(OPTIONS)) {
    ...
    public void removeExtraEmptyScreenWithoutAnimation(boolean snapToPage, boolean stripEmptyScreens){
        convertFinalScreenToEmptyScreenIfNecessary();
        if (hasExtraEmptyScreen()) {
            int emptyIndex = mScreenOrder.indexOf(EXTRA_EMPTY_SCREEN_ID);

            getPageIndicator().setAnimationType(AnimationType.NONE);

            if(snapToPage) {
                if (getNextPage() == emptyIndex) {
                    snapToPage(getNextPage() - 1, 0);
                } else {
            ...
            fadeAndRemoveEmptyScreen(0, FADE_EMPTY_SCREEN_DURATION,
                    null, stripEmptyScreens);

            getPageIndicator().setAnimationType(AnimationType.WORM);
        ...
        mRemoveEmptyScreenRunnable = new Runnable() {
            ...
            public void run() {
                if (hasExtraEmptyScreen()) {
                    ...
                    //This doesn't need to request the layout, so simply calling remove internal view
                    /*removeView(cl);*/
     ...
     */
    public void addInScreen(View child, long container, long screenId, int x, int y,
            int spanX, int spanY) {
        ...
        if(child instanceof BubbleTextView){
            ((BubbleTextView)child).setLongPressTimeout(Utilities.DESKTOP_ICON_LONG_PRESS_DURATION);
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        return super.onInterceptTouchEvent(ev) || isTransitionAnimationRunning();
    }
...
//            mQsbAlphaController.setAlphaAtIndex(alpha, QSB_ALPHA_INDEX_OVERLAY_SCROLL);
    ...
    public void computeScroll() {
        ...
        mWallpaperOffset.syncWithScroll();
        //If the state is changing to NORMAL from OPTIONS, do not call sync with scroll
        if(mLauncher.getStateManager().getToState() != NORMAL) {
            if (mFirstAnimatorSet == null && mSecondAnimatorSet == null)
                mWallpaperOffset.syncWithScroll();
        }
    }
...
//                            mQsbAlphaController.setAlphaAtIndex(alpha, QSB_ALPHA_INDEX_PAGE_SCROLL);
    ...
    public DragView beginDragShared(View child, DragSource source, ItemInfo dragObject,
        ...
        if (child instanceof BubbleTextView && !dragOptions.isAccessibleDrag) {
            BubbleTextView childView = (BubbleTextView) child;

            //Do not show popup menu on options state grouping
            if(!isOnGroupSelect(childView)) {
                PopupContainerWithArrow popupContainer = PopupContainerWithArrow
                        .showForIcon(childView);
                if (popupContainer != null) {
                    dragOptions.preDragCondition = popupContainer.createPreDragCondition();
                    mLauncher.getUserEventDispatcher()
                            .resetElapsedContainerMillis("dragging started");
                }
            }
        }
        ...
        DragView dv = mDragController.startDrag(b, dragLayerX, dragLayerY, source,
                dragObject, dragVisualizeOffset, dragRect, scale * iconScale, scale, dragOptions, child);
        dv.setIntrinsicIconScaleFactor(dragOptions.intrinsicIconScaleFactor);
        return dv;
    }

    public boolean isOnGroupSelect(View view){
        if(view instanceof BubbleTextView){
            BubbleTextView childView = (BubbleTextView)view;
            return mLauncher.drawCheck() && childView.isChecked();
        }
        return false;
    }
    ...
    public boolean acceptDrop(DragObject d) {
        ...
        if (d.dragSource != this) {
            ...
            if (mLauncher.isHotseat(dropTargetLayout)) {
            ...
            List<Point> resultPositionList = new ArrayList<Point>();
            if(foundCell && mDragController.isGroupDragging()) {
                mDragController.findTargetCellPositionsForGroupIcons(mTargetCell[0],
                        mTargetCell[1], screenId, resultPositionList);
            ...
            if (!foundCell || notEnoughSpace) {
                mDragController.getGroupIconManager().moveAllToOriginalPositions();
    ...
    //same function but using for group select icons
    public boolean addToExistingFolder(GroupIconView dragView, ItemInfo itemInfo, CellLayout target, int[] targetCell){
        if(!(itemInfo instanceof ShortcutInfo))
            return false;
        View dropOverView = target.getChildAt(targetCell[0], targetCell[1]);
        if(dropOverView instanceof FolderIcon){
            FolderIcon fi = (FolderIcon) dropOverView;
            if(fi.acceptDrop(itemInfo)){
                fi.onDrop(dragView, (ShortcutInfo)itemInfo, false);
                return true;
            }
        }
        return false;
    ...
    public void onDrop(final DragObject d, DragOptions options) {
        ...
        if (d.dragSource != this || mDragInfo == null) {
            ...
            if (dropTargetLayout != null && !d.cancelled) {
                ...
                boolean hasMovedIntoHotseat = mLauncher.isHotseat(dropTargetLayout);
                ...
                if (createUserFolderIfNecessary(cell, container,
                    ...
                    //Move group icons to the created or existing folder
                    if(mLauncher.drawCheck() && mDragController.isGroupDragging()){

                        //Check group icon list is not empty, and move them to target folder
                        if(mDragController.getGroupIconManager().getGroupIconCount() > 0){
                            mDragController.getGroupIconManager().addAllToExistingFolder(dropTargetLayout, mTargetCell);
                        }
                    }

                    //Check last state is options, and then do not go to normal state
                    if(!mLauncher.isInState(OPTIONS)) {
                ...
                //Move group icons to matching positions
                if(mLauncher.drawCheck() && mDragController.isGroupDragging()){

                    //Check group icon list is not empty, and move them to target positions
                    if(mDragController.getGroupIconManager().getGroupIconCount() > 0){
                        List<Point> resultPositionList = new ArrayList<Point>();
                        if(foundCell) {
                            mDragController.findTargetCellPositionsForGroupIcons(mTargetCell[0],
                        ...
                        if(!foundCell || notEnoughSpace) {
                            ...
                            mDragController.getGroupIconManager().createOrMoveShortcutsToTargetPositions(resultPositionList, screenId);
                    }
                }

                // if the widget resizes on drop
                if (foundCell && !notEnoughSpace && (cell instanceof AppWidgetHostView) &&
            ...
            //Check current state is not optionstate, and go to other state
            if(!mLauncher.isInState(OPTIONS)) {
    ...
    public void onDragExit(DragObject d) {
        ...
        //Move to original position of hotseat shortcuts anyway
        CellLayout hotSeatLayout = mLauncher.getHotseat().getLayout();
        hotSeatLayout.stopAnimations();
        hotSeatLayout.removeEmptySpaceOnHotSeat();
    }
   ...
   void mapPointFromSelfToHotseatLayout(Hotseat hotseat, float[] xy) {
    ...
    public void onDragOver(DragObject d) {
        //끌기가 허용되지 않았으면 여기서 끝낸다.
        if (!transitionStateShouldAllowDrop()) return;

        boolean oldDragDesktopToHotSeat = mIsDragDesktopToHotSeat;
        boolean oldDragHotSeatToHotSeat = mIsDragHotSeatToHotSeat;
        ...
        mIsDragHotSeatToHotSeat = mDragTargetLayout != null && mDragTargetLayout.isHotseat() && item.isContainerHotSeat();
        ...
        if (setDropLayoutForDragObject(d, mDragViewVisualCenter[0], mDragViewVisualCenter[1])) {
            if (mLauncher.isHotseat(mDragTargetLayout)) {
                mSpringLoadedDragController.cancel();
            } else {
        ...
        if (mDragTargetLayout != null) {
            ...
            boolean dragToHotSeat = mDragTargetLayout.isHotseat();
            if(!dragToHotSeat) {
            ...
            //Get if the drag is from desktop to hotseat
            mIsDragDesktopToHotSeat =
                    mDragTargetLayout.isHotseat() && !item.isContainerHotSeat();

            if(!mDragTargetLayout.isHotseat()) {
            ...
            //Check child count is not over max hotseat app count
            else if(!(mDragTargetLayout.getHotSeatChildCount() == MAX_COUNT_HOTSEAT && mIsDragDesktopToHotSeat)){
                mDragTargetLayout.visualizeDropLocation(child, mOutlineProvider,
                        mTargetCell[0], mTargetCell[1], item.spanX, item.spanY, false, d);
            }

            CellLayout hotSeatCellLayout = mLauncher.getHotseat().getLayout();
            int maxHotSeatChildCount = MAX_COUNT_HOTSEAT;
            if(mIsDragDesktopToHotSeat)
                maxHotSeatChildCount --;
            if(mTargetCell[0] != -1 &&
                    hotSeatCellLayout.getHotSeatChildCount() <= maxHotSeatChildCount) {
                //Case dragging desktop to hotseat
                if (!oldDragDesktopToHotSeat && mIsDragDesktopToHotSeat) {
                    //Drag start
                    hotSeatCellLayout.onDesktopToHotSeatDragOver(mTargetCell[0]);
                    mDragHotSeatIndex = mTargetCell[0];
                } else if (oldDragDesktopToHotSeat && mIsDragDesktopToHotSeat) {
                    //Drag move
                    if (mDragHotSeatIndex != mTargetCell[0]) {
                        hotSeatCellLayout.onDesktopToHotSeatDragOver(mTargetCell[0]);
                        mDragHotSeatIndex = mTargetCell[0];
                    }
                } else if (oldDragDesktopToHotSeat) {
                    //Drag end
                    hotSeatCellLayout.onDesktopToHotSeatDragLeave();
                }

                //Case dragging hotseat to hotseat
                if (!oldDragHotSeatToHotSeat && mIsDragHotSeatToHotSeat) {
                    //Drag start
                    hotSeatCellLayout.onHotSeatToHotSeatDragOver(item.cellX, mTargetCell[0]);
                    mDragHotSeatIndex = mTargetCell[0];
                } else if (oldDragHotSeatToHotSeat && mIsDragHotSeatToHotSeat) {
                    //Drag move
                    if (mDragHotSeatIndex != mTargetCell[0]) {
                        hotSeatCellLayout.onHotSeatToHotSeatDragOver(item.cellX, mTargetCell[0]);
                        mDragHotSeatIndex = mTargetCell[0];
                    }
                } else if (oldDragHotSeatToHotSeat) {
                    //Drag end
                    hotSeatCellLayout.onHotSeatToHotSeatDragLeave(item.cellX);
                }
            }
    ...
    private boolean setDropLayoutForDragObject(DragObject d, float centerX, float centerY) {
        ...
        float startX = ((d.x - d.xOffset) - d.dragView.getMeasuredWidth() * (d.dragView.getTargetScale() - 1) * 0.5f);
        int nextPage = getNextPage();
        if (layout == null && !isPageInTransition()) {
            // Check if the item is dragged over left page

            //mTempTouchCoordinates[0] = Math.min(centerX, d.x);
            mTempTouchCoordinates[0] = startX;
        ...
        if (layout == null && !isPageInTransition()) {
            ...
            mTempTouchCoordinates[0] = startX + d.dragView.getTargetScale() * d.dragView.getWidth();
            mTempTouchCoordinates[1] = d.y;
            layout = verifyInsidePage(nextPage + (mIsRtl ? -1 : 1), mTempTouchCoordinates);
        }
    ...
    private void onDropExternal(final int[] touchXY, final CellLayout cellLayout, DragObject d) {
        ...
        final long container = mLauncher.isHotseat(cellLayout) ?
                LauncherSettings.Favorites.CONTAINER_HOTSEAT :
                    LauncherSettings.Favorites.CONTAINER_DESKTOP;
        final long screenId = getIdForScreen(cellLayout);
        if (!mLauncher.isHotseat(cellLayout)
        ...
        if (info instanceof PendingAddItemInfo) {
            ...
            if (touchXY != null) {
                ...
                if (createUserFolderIfNecessary(view, container, cellLayout, mTargetCell, distance,
                    ...
                    if(mLauncher.drawCheck() && mDragController.isGroupDragging()){
                        ...
                        //Check group icon list is not empty, and move them to target folder
                        if(mDragController.getGroupIconManager().getGroupIconCount() > 0){
                            mDragController.getGroupIconManager().addAllToExistingFolder(cellLayout, mTargetCell);
                        }
                    }
                    return;
                }
            ...
            if(mLauncher.drawCheck() && mDragController.isGroupDragging()){
                ...
                if(mDragController.getGroupIconManager().getGroupIconCount() > 0){
                    List<Point> resultPositionList = new ArrayList<Point>();
                    mDragController.findTargetCellPositionsForGroupIcons(mTargetCell[0], mTargetCell[1], screenId, resultPositionList);
                    mDragController.getGroupIconManager().createOrMoveShortcutsToTargetPositions(resultPositionList, screenId);
                }
            }
        }
    ...
    //Return the current and next children

    /**
     * 미리보기 Animation 을 진행하기 위한 View들(첫페지, 두번째 페지)을 얻는다,
     * @param transitionLayout TransitionLayout
     */
    public boolean getChildrenForPreview(TransitionLayout transitionLayout){
        //If current page is last page, and last page is empty, set first page as previous page, and next pas as current page
        boolean isLastPageAndEmpty = isLastPage() && ((CellLayout)getPageAt(mCurrentPage)).isEmptyCellLayout();
        int firstPage = isLastPageAndEmpty? mCurrentPage - 1 : mCurrentPage;
        int secondPage = isLastPageAndEmpty? mCurrentPage: mCurrentPage + 1;

        if(getPageAt(firstPage) != null){
            transitionLayout.setFirstChild((CellLayout)getPageAt(firstPage));
        }
        else{
            transitionLayout.setFirstChild(null);
        }

        if(getPageAt(secondPage) != null){
            transitionLayout.setSecondChild((CellLayout)getPageAt(secondPage));
        }
        else{
            transitionLayout.setSecondChild(null);
        }

        return isLastPageAndEmpty;
    }

    public boolean isLastPageEmpty(){
        int lastPage = getPageCount() - 1;
        if(getPageAt(lastPage) == null)
            return false;
        return ((CellLayout)getPageAt(lastPage)).isEmptyCellLayout();
    }

    //Hide the views
    public void hideNeighbors(){
        if(getPageAt(mCurrentPage - 1) != null)
            getPageAt(mCurrentPage - 1).setVisibility(INVISIBLE);
    ...
    public void showNeighbors(){
        if(getPageAt(mCurrentPage - 1) != null)
            getPageAt(mCurrentPage - 1).setVisibility(VISIBLE);
    ...
    public boolean isDragDesktopToHotSeat(){
        return mIsDragDesktopToHotSeat;
    }

    public void initializeValues(){
        mIsDragDesktopToHotSeat = false;
        mIsDragHotSeatToHotSeat = false;
    }

    public int getLayoutTransitionOffsetForPage(int index) {
        if(mFirstAnimatorSet != null || mSecondAnimatorSet != null)
            return 0;
        return super.getLayoutTransitionOffsetForPage(index);
    }

    /**
     * 현재의 Transition animation을 취소한다.
     * 동작은 본래 위치로 되돌아가는 animation이다.
     */
    private void cancelCurrentTransitionAnimation() {
        if(mFirstAnimatorSet == null && mSecondAnimatorSet == null)
            return;
        ...
        AnimatorListener animatorListener = new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //모든 animator들에서 AnimationEnd사건이 발생하게 되면
                //Animation마감동작을 진행한다.
                if(mRunningAnimatorCount == 0){
                    doAnimationEndAction(-1);
                }
            }
        ...
        //Animator들의 reverse를 호출하여 후진 animation을 진행한다.
        mRunningAnimatorCount = 0;
        if(mFirstAnimatorSet != null) {
            mRunningAnimatorCount ++;
            mFirstAnimatorSet.addListener(animatorListener);
            mFirstAnimatorSet.reverse();
        }
        if(mSecondAnimatorSet != null) {
            mRunningAnimatorCount ++;
            mSecondAnimatorSet.addListener(animatorListener);
            mSecondAnimatorSet.reverse();
        }
        if(mWallpaperMoveAnimator != null) {
            mRunningAnimatorCount ++;
            mWallpaperMoveAnimator.addListener(animatorListener);
            mWallpaperMoveAnimator.reverse();
        }
    }

    /**
     * 현재의 Transition animation을 계속해서 진행한다.
     * @param finalPage 목표페지
     */
    private void finishCurrentTransitionAnimation(int finalPage) {
        if(mFirstAnimatorSet == null && mSecondAnimatorSet == null)
            return;
        ...
        AnimatorListener animatorListener = new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //Animation마감동작을 진행한다.
                if(mRunningAnimatorCount == 0){
                    doAnimationEndAction(finalPage);
                }
            }
     ...
     * @param gotoPage 목표페지
     */
    private void doAnimationEndAction(int gotoPage){
        /*-- 상태들을 복귀한다. --*/
        if (mFirstAnimatorSet != null) {
            if (mAnimateFirstView != null) {
                mAnimateFirstView.restoreState();
        ...
        mTouchInterceptAnimation = false;

        if(gotoPage >= 0){
            setCurrentPage(gotoPage);
        }
        //Scroll위치 다시 계산
        mCurrentX = getUnboundedScrollX();
    }

    /**
     * 전환 animation이 진행중인가?
     */
    public boolean isTransitionAnimationRunning() {
        if(mFirstAnimatorSet != null)
            return true;
        if(mSecondAnimatorSet != null)
            return true;
    ...
    public int getScrollXOnAnimation() {
        if(mFirstAnimatorSet != null || mSecondAnimatorSet != null) {
            return (int) (getScrollX() - mLastOffsetX);
        }
        return getScrollX();
    }
    ...
    public void reloadTransitionPref(){
