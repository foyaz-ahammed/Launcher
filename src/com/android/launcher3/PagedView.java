 ...
 * 페지화된 View클라스
 *
 * Home 화면의 Workspace 나 등록부보기와 같이 여러개의 페지를 보여줄때 리용한다.
 * @param <T> {@link PageIndicator}
 *
 * @see Workspace
 * @see FolderPagedView
 */
public abstract class PagedView<T extends View & PageIndicator> extends ViewGroup {
    ...
    protected int mMaxScrollX;      //Scroll 최대거리
    ...
    //OverScroll 진행중이였는가? (마지막페지에 도달해서 scroll을 더 진행하였을때 overscroll발생)
    ...
    int validateNewPage(int newPage) {
    ...
    boolean isTouchPointInViewportWithBuffer(int x, int y) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        //If preview animation is running, skip the touch event
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                boolean finishedScrolling = (mScroller.isFinished() || xDist < mTouchSlop / 3);

                //For workspace, scroller is not used, instead checking animation
                if(this instanceof Workspace){
                    finishedScrolling = !((Workspace)this).isTransitionAnimationRunning();
                }
    ...
    /**
     * 해당 페지의 scroll위치를 돌려준다.
     * 첫페지에서는 scroll 위치가 0으로 된다.
     *
     * @param index 페지번호
     */
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        Log.w("PagedView-GenericMotion", "point: " + event.getX() + " action: " + event.getAction());
    ...
    public boolean snapToPage(int whichPage, int delta, int duration, boolean immediate,
            TimeInterpolator interpolator) {
        if (mFirstLayout) {
