...
public class DragView extends View {
    ...
    public DragView(Launcher launcher, Bitmap bitmap, int registrationX, int registrationY,
                    final float initialScale, final float scaleOnDrop, final float finalScaleDps, View cellView) {
        super(launcher);
        mLauncher = launcher;
        mDragLayer = launcher.getDragLayer();
        mDragController = launcher.getDragController();

        //Set Cell View to scale to original after animation ends
        mDragLayer.setCellView(cellView);

        iconProvider = IconProvider.newInstance(launcher);

        float tempScale = ((bitmap.getWidth() + finalScaleDps) / bitmap.getWidth());

        //If the icon is in folder, or the state is option state, scale plus
        if(cellView instanceof BubbleTextView){
            BubbleTextView bubbleTextView = (BubbleTextView)cellView;
            if(bubbleTextView.isInFolder()){
                Folder folder = bubbleTextView.getParentFolder();
                if(folder != null){
                    tempScale *= folder.getScaleX();
                }
            }
            else{
                tempScale *= launcher.getWorkspace().getScaleX();
            }
        }
        mTargetScale = tempScale;
        ...
        mAnim.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                setScaleX(initialScale + (value * (mTargetScale - initialScale)));
                setScaleY(initialScale + (value * (mTargetScale - initialScale)));
                if (sDragAlpha != 1f) {
