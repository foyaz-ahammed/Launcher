...
public class DragController implements DragDriver.EventListener, TouchController {
    ...
    //Point list(queue) for afterimage effect
    private LinkedList<Point> mPointList = new LinkedList<>();
    ...
    GroupIconManager mGroupIconManager;
    boolean mIsGroupDragging = false;
    public boolean mGroupIconMergeAnimation = false;

    //List of running animator
    private List<Animator> mRunningAnimatorList = new ArrayList<Animator>();
    private final static int DRAG_BEGINNING = 100;
    private final static int DRAG_RUNNING = 101;
    private final static int DRAG_ENDED = 102;
    private int mDraggingStatus = DRAG_ENDED;
    ...
    public DragController(Launcher launcher) {
        ...
        mGroupIconManager = new GroupIconManager(launcher);
        mGroupIconManager.setDragController(this);
    }

    public GroupIconManager getGroupIconManager(){
        return mGroupIconManager;
    }
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY,
            ...
            float initialDragViewScale, float dragViewScaleOnDrop, DragOptions options, View cellView) {
        ...
        final DragView dragView = mDragObject.dragView = new DragView(mLauncher, b, registrationX,
                registrationY, initialDragViewScale, dragViewScaleOnDrop, scaleDps, cellView);
        dragView.setItemInfo(dragInfo);

        if(mLauncher.drawCheck() && cellView instanceof BubbleTextView && ((BubbleTextView) cellView).isChecked()){
            mIsGroupDragging = true;
            mGroupIconManager.setMainView((BubbleTextView) cellView);
            mGroupIconManager.getAllCheckedViews();

            float scaleToFolder = 0;

            //Use folder scale to drag view
            if(mGroupIconManager.isMainViewInFolder()){
                scaleToFolder = mGroupIconManager.getMainView().getParentFolder().getScaleX();
            }

            mGroupIconManager.hideAndMakeInitialDragViews(dragLayerX, dragLayerY, initialDragViewScale, scaleToFolder);
        }
        else {
            mGroupIconManager.clear();
            mGroupIconManager.setMainView(null);
        }

        //Set flag group icon merge animation is in progress, or ended
        if(mGroupIconManager.getGroupIconCount() > 0){
            mGroupIconMergeAnimation = true;
        }
    ...
    //Save translate x, y value to point list
    private void addPositionFromView(View view){
        Point point = new Point((int)view.getTranslationX(), (int)view.getTranslationY());
        mPointList.add(point);
        if(mPointList.size() == mGroupIconManager.getGroupIconCount() + 1){
            mPointList.remove();
        }
    }
    ...
    private void callOnDragStart() {
        ...
        mLauncher.playDragLabelAnimation(true);
    }
    ...
    private void endDrag() {
        ...
        mIsGroupDragging = false;

        mFlingToDeleteHelper.releaseVelocityTracker();
    }

    public boolean isGroupDragging(){
        return mIsGroupDragging;
    }

    //Find the target positions when group dragging
    public void findTargetCellPositionsForGroupIcons(int firstTargetCellX, int firstTargetCellY, long screenId,
            List<Point> resultPositionList){
        CellLayout cellLayout = mLauncher.getWorkspace().getScreenWithId(screenId);

        if(cellLayout == null)
            return;

        int column_count = mLauncher.getDeviceProfile().inv.numColumns;
        int row_count = mLauncher.getDeviceProfile().inv.numRows;
        ...
        for (int j = 0; j < row_count; j ++){
            ...
            for (int i = 0; i < column_count; i ++){
                if(j < firstTargetCellY)
                    continue;
                if(j == firstTargetCellY && i <= firstTargetCellX)
                    continue;

                if(cellLayout.isOccupied(i, j))
                    continue;

                resultPositionList.add(new Point(i, j));
                if(resultPositionList.size() == mGroupIconManager.getGroupIconCount()) {
                    finished = true;
                    break;
                }
            }
        ...
        //Check found position count is smaller than expected count, and then clear it
        if(resultPositionList.size() < mGroupIconManager.getGroupIconCount())
    ...
    private void callOnDragEnd() {
        ...
        mLauncher.playDragLabelAnimation(false);
    }
    ...
    public void onDriverDragMove(float x, float y) {
        ...
        if(mLastTouch[0] == dragLayerPos[0] && mLastTouch[1] == dragLayerPos[1])
            return;
    ...
    private void handleMoveEvent(int x, int y) {
        //First move to the old track positions of drag view
        if(mIsGroupDragging && !mGroupIconMergeAnimation){
            mGroupIconManager.dragMoveWithoutAnimation(mPointList);
        }

        mDragObject.dragView.move(x, y);
        addPositionFromView(mDragObject.dragView);

        //Animate to the position if the move doesn't continue
        if(mIsGroupDragging && !mGroupIconMergeAnimation){
            mGroupIconManager.dragMoveWithAnimation(mDragObject.dragView.getTranslationX(), mDragObject.dragView.getTranslationY());
        }
    ...
    public void stopAllRunningAnimations(){
        for (int i = 0; i < mRunningAnimatorList.size(); i ++){
            Animator animator = mRunningAnimatorList.get(i);
            animator.end();
        }
        mRunningAnimatorList.clear();
    }
    ...
    private void checkTouchMove(DropTarget dropTarget) {
        if (dropTarget != null) {
            if (mLastDropTarget != dropTarget) {
                ...
                mDraggingStatus = DRAG_BEGINNING;
            }

            if(mDraggingStatus == DRAG_RUNNING && mRunningAnimatorList.size() > 0){
                stopAllRunningAnimations();
                mGroupIconMergeAnimation = false;
            }

            mDraggingStatus = DRAG_RUNNING;
    ...
    private void drop(DropTarget dropTarget, Runnable flingAnimation) {
        ...
        if(dropTarget instanceof Workspace){
            ((Workspace) dropTarget).initializeValues();
        }
    }
    ...
    public void addToRunningAnimatorList(Animator animator){
        mRunningAnimatorList.add(animator);
    }
