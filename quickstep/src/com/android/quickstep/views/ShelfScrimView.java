...
public class ShelfScrimView extends ScrimView {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (mRemainingScreenColor != 0) {
            ...
            //Do not draw the screen path(it looks rather good now)
//            canvas.drawPath(mRemainingScreenPath, mPaint);
            canvas.translate(0, offset);
        }

        mPaint.setColor(mShelfColor);
        //Do not draw the roundrect same size and position of drawer, set it full screen size
//        onDrawRoundRect(canvas, 0, mShelfTop, getWidth(), height + mRadius,
//                mRadius, mRadius, mPaint);
        onDrawRoundRect(canvas, 0, 0, getWidth(), getHeight(),
                0, 0, mPaint);
    }
