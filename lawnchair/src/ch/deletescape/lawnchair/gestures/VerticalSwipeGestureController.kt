...
class VerticalSwipeGestureController(private val launcher: Launcher) : TouchController, SwipeDetector.Listener {
    ...
    override fun onControllerInterceptTouchEvent(ev: MotionEvent): Boolean {
        ...
        if(workspaceIsOnTransitionAnimation())
            return false;
    ...
    private fun canInterceptTouch(): Boolean {
        return AbstractFloatingView.getTopOpenView(launcher) == null &&
               (launcher.isInState(LauncherState.NORMAL) && !launcher.workspace.isTransitionAnimationRunning)
    }

    private fun workspaceIsOnTransitionAnimation(): Boolean {
        return (launcher.isInState(LauncherState.NORMAL) && launcher.workspace.isTransitionAnimationRunning);
    }
