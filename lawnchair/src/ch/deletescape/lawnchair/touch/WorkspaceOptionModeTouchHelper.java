...
public class WorkspaceOptionModeTouchHelper {
    ...
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        ...
        Log.w("WorkspaceTouchHelper-distouch", "point: " + motionEvent.getX() + " action: " + motionEvent.getAction());
        ...
        if (motionEvent.getAction() == MotionEvent.ACTION_UP) {
            ...
            if ((Math.abs(x) > mMinSnapDistance || mVelocity >= ((float) mMinSnapVelocity)) && !(mIsStillPossibleClick && isPossibleClick)) {
                ...
                if (mIsStillPossibleClick && isPossibleClick(motionEvent)) {
                    ...
                    mLauncher.getWorkspace().getPageIndicator().setAnimationType(AnimationType.NONE);
                    int page = getNextPage(mTouchDownWorkspaceCurrentPage,
                            mTouchDownX > mLauncher.getDeviceProfile().widthPx / 2);
                    if(page < workspace.getPageCount() - 1) {
                        workspace.snapToPage(page, 0);
                    }

                    //Remove the last empty screen
                    mLauncher.getWorkspace().removeExtraEmptyScreenWithoutAnimation(false, true);
