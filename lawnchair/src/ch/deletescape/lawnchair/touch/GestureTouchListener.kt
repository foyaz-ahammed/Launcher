...
open class GestureTouchListener(context: Context) : View.OnTouchListener {
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (clickPossible && launcher.isInState(LauncherState.OPTIONS)) {
                    //First close the transition layout view
                    val view = AbstractFloatingView.getTopOpenView(launcher);
                    if(view is TransitionLayout){
                        val transitionView : TransitionLayout = view;
                        transitionView.handleClose(false)
                    }

                    launcher.workspace.removeExtraEmptyScreenWithoutAnimation(true, true)
    ...
    fun onLongPress() {
        if (launcher.isInState(LauncherState.NORMAL)) {
            launcher.workspace.addExtraEmptyScreen()
//            launcher.workspace.pageIndicator.playAnimation(true, 300)
