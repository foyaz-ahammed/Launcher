...
open class LawnchairLauncher : NexusLauncherActivity(),
        ...
        if(requestCode == REQUEST_PERMISSION_ALL){
            ...
            if (showRationale){
                AlertDialog.Builder(this)
                        ...
                        .setMessage(R.string.content_permission_required)
                        .setPositiveButton(android.R.string.ok) { _, _ ->
                            ActivityCompat.requestPermissions(this@LawnchairLauncher, PERMISSIONS, REQUEST_PERMISSION_ALL) }
                        .setCancelable(false)
                        .create().apply {
                            show()
                            applyAccent()
            ...
            val calendarPermissionIndex = PERMISSIONS.indexOf(Manifest.permission.READ_CALENDAR)
            if(calendarPermissionIndex != -1 && grantResults.size > calendarPermissionIndex
               && grantResults[calendarPermissionIndex] != -1) {
                Launcher.getLauncher(this).registerCalendarObserver()
            }
    ...
    companion object {
        ...
        const val REQUEST_PERMISSION_ALL = 666
